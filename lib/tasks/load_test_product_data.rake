...
# e.g. usage rake command to run:  rake db:drop db:create db:schema:load db:seed app:load_demo_data app:load_demo_products

namespace :app do
  desc <<-DESC
    Load product testing data.
    Run using the command 'rake db:drop db:create db:schema:load db:seed app:load_demo_data app:load_demo_products'
  DESC
require 'csv'
task :load_demo_products => :environment do
